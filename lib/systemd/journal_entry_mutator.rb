...
module Systemd
  ...
  class JournalEntryMutator
    ...
    def initialize(**options)
      init_options_from_hash(options)
      @map = invert_field_map(@field_map)
      @map_src_fields = @field_map.keys
    end
    ...
    def run(entry)
      return map_fields(entry) if @field_map_strict
    ...
    def format_fields(entry, mapped = nil)
      ...
      entry.each do |fld, val|
        ...
        fld = fld.gsub(/\A_+/, "") if @fields_strip_underscores
        fld = fld.downcase if @fields_lowercase
    ...
    def init_options_from_hash(opts)
      merged = DEFAULT_OPTS.merge(opts.keep_if { |k, _| DEFAULT_OPTS.key?(k) })
      validate_options(merged)
      merged.each { |k, v| instance_variable_set("@#{k}", v) }
    end
